...
void HierarchyLayer::OnAttach()
{
  ...
  m_BoneContextMenu.Register(
      {.renderOn = ContextMenu::PopupContext::ITEM,
       .items    = {
           {
                  .name    = "Add child",
                  .onClick = [](void* data) { ServiceLocator::Get<Rigger>()->NewBone(ToUInt64(data)); },
           },
           {
                  .name    = "Remove child",
                  .onClick = [](void* data) { ServiceLocator::Get<Rigger>()->RemoveBone(ToUInt64(data)); },
           },
           {
                  .name = "Rename",
                  .onRenderItem =
                   [](ContextMenu::Item* item, void* data)
               {
                 ...
                 if (ImGui::MenuItem(item->name, item->shortcut, item->selected, item->enabled ? *item->enabled : true))
                   ServiceLocator::Get<ECS::Registry>()->Get<EBone, CFlags>(ToUInt64(data))->rename = true;
               },
           },
       }});
  ...
  m_Hierarchy.OnRenderItem(
      [](Hierarchy::Item* item, void* data)
      {
        ...
        else
        {
          ...
          ImGui::InputText(inputId.c_str(), cHierarchy->name, IM_ARRAYSIZE(cHierarchy->name),
  ...
  m_ContextMenu.Register({
      ...
      .items    = {{
             .name     = "New bone",
             .shortcut = "Ctrl B",
             .onClick  = [](void* data) { ServiceLocator::Get<Rigger>()->NewBone(0); },
      }},
  });
}
